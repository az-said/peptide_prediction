...
function buildInterpretation(peptide: Peptide): string[] {
  ...
  if (peptide.s4predHelixPrediction === 1) {
    const pct = peptide.s4predHelixPercent != null
  ...
  // ── Aggregation risk ───────────────────────────────────────────
  const maxAgg = peptide.tangoAggMax ?? (peptide.tango?.agg ? Math.max(...peptide.tango.agg) : null);
  if (maxAgg != null) {
    if (maxAgg > 50) {
      paragraphs.push(
        ...
        `are considered aggregation-prone and may form amyloid-like structures under ` +
  ...
  if (peptide.sswPrediction === 1 || (peptide.s4predSswPrediction ?? 0) === 1) {
    paragraphs.push(
      ...
      `helical and beta-strand states, a hallmark of amyloidogenic peptides.`,
    );
    ...
    if (peptide.ffSswFlag === 1) {
      paragraphs.push(
        ...
        `in the highest-risk category for amyloid fibril formation.`,
      );
    }
