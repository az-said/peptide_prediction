 ...
 */
export function mapApiRowToPeptide(
  row: ApiPeptideRow | Record<string, any>,
  source: string = "API"
): Peptide {
  ...
  const id = String(
    row.id || row.Entry || row.entry || row.Accession || row.accession || ""
  ).trim();
  if (!id) {
    console.warn(
      "[mapApiRowToPeptide] No valid ID found in row. Available keys:",
      Object.keys(row)
    );
  ...
  const sswHelixPct = num(
    row.sswHelixPercentage ?? row.sswHelixPct ?? row["SSW helix percentage"],
    true
  );
  const sswBetaPct = num(
    row.sswBetaPercentage ?? row.sswBetaPct ?? row["SSW beta percentage"] ?? row.betaPercent,
    true
  );
  ...
  const tango: TangoCurves | undefined = (() => {
    const agg = (row.tangoAggCurve ||
      row.tangoAgg ||
      row["Tango Aggregation curve"] ||
      extra["Tango Aggregation curve"]) as number[] | undefined;
    const beta = (row.tangoBetaCurve ||
      row.tangoBeta ||
      row["Tango Beta curve"] ||
      extra["Tango Beta curve"]) as number[] | undefined;
    const helix = (row.tangoHelixCurve ||
      row.tangoHelix ||
      row["Tango Helix curve"] ||
      extra["Tango Helix curve"]) as number[] | undefined;
    const turn = (row.tangoTurn || row["Tango Turn curve"] || extra["Tango Turn curve"]) as
      | number[]
      | undefined;
  ...
  const s4pred: S4predInfo | undefined = (() => {
    const pH = (row.s4predPHCurve || row["S4PRED P_H curve"] || extra["S4PRED P_H curve"]) as
      | number[]
      | undefined;
    const pE = (row.s4predPECurve || row["S4PRED P_E curve"] || extra["S4PRED P_E curve"]) as
      | number[]
      | undefined;
    const pC = (row.s4predPCCurve || row["S4PRED P_C curve"] || extra["S4PRED P_C curve"]) as
      | number[]
      | undefined;
    const ssPrediction = (row.s4predSsPrediction ||
      row["S4PRED SS prediction"] ||
      extra["S4PRED SS prediction"]) as string[] | undefined;
    const helixSegments = toSegments(row.s4predHelixFragments || row["Helix fragments (S4PRED)"]);
    const betaSegments = toSegments(row.s4predSswFragments || row["SSW fragments (S4PRED)"]);
    if (
      (pH && pH.length) ||
      (pE && pE.length) ||
      (pC && pC.length) ||
      (ssPrediction && ssPrediction.length) ||
      (helixSegments?.length ?? 0) > 0 ||
      (betaSegments?.length ?? 0) > 0
    ) {
  ...
  const s4predHelixPrediction = num(
    row.s4predHelixPrediction ?? row["Helix prediction (S4PRED)"],
    true
  );
  ...
  const s4predHelixFragmentsTop = toSegments(
  ...
  const ffHelixScore = num(
    row.ffHelixScore ?? row["FF-helix score"] ?? row["FF-Helix score"],
    true
  );
  const ffSswFlag = num(row.ffSswFlag ?? row["FF-Secondary structure switch"], true);
  const ffSswScore = num(
    row.ffSswScore ?? row["FF-Secondary structure switch score"] ?? row["FF-SSW score"],
    true
  );
 ...
 */
export function mapApiRowsToPeptides(
  rows: (ApiPeptideRow | Record<string, any>)[],
  source: string = "API"
): Peptide[] {
