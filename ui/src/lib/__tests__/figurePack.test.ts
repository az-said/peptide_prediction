...
describe("generateMethodsSVG", () => {
  ...
  it("includes PVL citation", () => {
    ...
    // Citation footer points at the real source repo, not the old pvl.example placeholder.
    expect(svg).toContain("Generated by PVL");
    expect(svg).toContain("github.com/saidaz24-meet/peptide_prediction");
  });

  it("embeds the reproducibility permalink when provided", () => {
    // Use an ampersand-free URL so we can substring-match the raw text.
    // The renderer XML-escapes `&` to `&amp;`, which is correct for SVG but
    // makes a literal substring assertion brittle.
    const url = "https://example.org/results/abc-def-123";
    const svg = generateMethodsSVG(DEFAULT_THRESHOLDS, 2, { permalinkURL: url });
    expect(svg).toContain("Reproduce this figure");
    expect(svg).toContain(url);
  });

  it("renders the supplied version label", () => {
    const svg = generateMethodsSVG(DEFAULT_THRESHOLDS, 1, { version: "0.2.3" });
    expect(svg).toContain("v0.2.3");
  });
});
