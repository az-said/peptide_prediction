...
def _run_tango_for_single_sequence(df: pd.DataFrame, entry_id: str, seq: str) -> None:
    ...
    try:
        ...
        try:
            ...
            log_warning(
                "tango_parse_unexpected", f"Unexpected error during TANGO output processing: {e}"
            )
...
def _run_s4pred_provider(df: pd.DataFrame) -> None:
    ...
    try:
        ...
        log_info(
            "s4pred_complete",
            f"S4PRED completed: {stats.get('parsed_ok', 0)}/{stats.get('requested', 0)}",
            **stats,
        )
...
def _build_provider_status_meta(
    ssw_hits: int, tango_ran: bool, s4pred_ran: bool = False
) -> Dict[str, Any]:
    """Build provider status metadata for single sequence prediction."""
    tango_status = (
        "AVAILABLE" if ssw_hits > 0 else ("OFF" if not settings.USE_TANGO else "UNAVAILABLE")
    )
    tango_reason = (
        None
        if ssw_hits > 0
        else ("TANGO not enabled" if not settings.USE_TANGO else "No TANGO output")
    )
    s4pred_status = (
        "AVAILABLE" if s4pred_ran else ("OFF" if not settings.USE_S4PRED else "UNAVAILABLE")
    )
    s4pred_reason = (
        None
        if s4pred_ran
        else ("S4PRED not enabled" if not settings.USE_S4PRED else "No S4PRED output")
    )
...
def _compute_reproducibility_primitives(entry_id: str, seq: str, ssw_hits: int) -> tuple:
    """
    ...
    provider_status_summary = {
        "tango": {
            "status": "AVAILABLE"
            if ssw_hits > 0
            else ("OFF" if not settings.USE_TANGO else "UNAVAILABLE"),
            "requested": 1 if settings.USE_TANGO else 0,
            "parsed_ok": ssw_hits,
            "parsed_bad": 0,
        }
