...
On **2026-06-29 Peleg replied** (recorded in [`DECISIONS.md` ADR-021](https://github.com/az-said/peptide_prediction/blob/main/docs/active/DECISIONS.md)). Her verbatim guidance: *"the motif one. make sure its good for researchers. magenta confirmed. delete per meeting. other than that, keep everything as is. its fine. next dev will take care of the specifics."*

That close-out means: **5 of 8 OQs are resolved, 3 are deferred** (open, but Peleg does *not* consider them publication-blocking). The deferred three live on as Tier-4 polish entries in [`BACKLOG.md`](https://github.com/az-said/peptide_prediction/blob/main/docs/active/BACKLOG.md).
...
- **Source:** `PELEG-Q-FIX-022`, tracked in [`ROADMAP.md`](https://github.com/az-said/peptide_prediction/blob/main/docs/active/ROADMAP.md) Round-2 (Wave 2.7) and the Wave C discussion list (FIX-022, "charge handling: absolute vs signed"). Listed as Peleg co-design item **D6**.
