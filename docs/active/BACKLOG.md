...
- **ISSUE-034 — precompute TANGO subprocess** (`KNOWN_ISSUES.md`). Precompute path reports `tango.status: AVAILABLE, requested: 0`. Live `/api/predict` works fine; only the precompute pipeline silently skips TANGO. Root cause likely in `process_upload_dataframe`'s cache / row-validation. **Effort: 1 day diagnosis + fix.**
